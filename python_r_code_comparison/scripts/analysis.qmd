...
This morning (***August 20, 2022***), I saw a **LinkedIn** post where the user compares `R` and `Python` solutions. So, I decided to play around with the data in both `R` and `Python` to see the results myself. Below is an example code in R and Python.
...
```{r}
# Import the data
data_raw <- read_csv("python_r_code_comparison/data.csv", show_col_types = FALSE)
...
#| echo: true
# Import the data
raw = pd.read_csv('python_r_code_comparison/data.csv')
